...
class RandomImageSource(ImageSource):
    ...
    def build_arr(self):
        self.total_frames = self.total_frames if self.total_frames else len(
            self.filelist)
        self.arr = np.zeros(
        ...
        for i in range(self.total_frames):
            ...
            self.arr[i] = cv2.resize(im, (self.shape[1], self.shape[0]))
    ...
    def build_arr(self):
        if not self.total_frames:
            ...
            for fname in tqdm.tqdm(self.filelist, desc="Loading videos for natural", position=0):
                ...
                local_arr = np.zeros(
                    (frames.shape[0], self.shape[0], self.shape[1]) + ((3,) if not self.grayscale else (1,)))
